(() => {
	...
	newPost.addEventListener('submit', event => {
		...
		const tweetValue = document.getElementById('new-post-text').value;
		window.tweet.post(tweetValue);
		tweetValue.value = "";
	});
	...
	// Event се хвърля за всеки един tweet
	tweetsDB.on('child_added', data => {
		// Logic when new tweet is added
		postContainer.innerHTML += post(data);
 
		// Само трябва да се оправи, правилно да се взима подходящия бутон, за който да викаме events.
		const likeButton = postContainer.querySelector(`[data-id=${data.key}]`)[0];
		const dislikeButton = postContainer.querySelector(`[data-id=${data.key}]`)[1];

		likeButton.addEventListener("click", () => {
			window.tweet.incrementLikes(data.key);
		})
		
		dislikeButton.addEventListener("click", () => {
			window.tweet.incrementDislikes(data.key);
		})
